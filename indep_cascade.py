...
def fraction_activated(seed_set, message=None, M_end=None, M_start=1, g=g, target_nodes=target_nodes):
	"""
	...
	for N in range(M_start, M_end):
		...
		activated_fraction = float(len(set([x for y in activated_nodes for x in y]).intersection(target_nodes)))/N
